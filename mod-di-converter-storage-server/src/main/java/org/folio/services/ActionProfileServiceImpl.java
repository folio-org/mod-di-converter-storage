...
public class ActionProfileServiceImpl
  ...
  private static final String INVALID_RECORD_TYPE_LINKED_MAPPING_PROFILE_TO_ACTION_PROFILE =
    "Mapping profile '%s' can not be linked to this Action profile. "
    + "ExistingRecordType and FolioRecord types are different";
  private static final String INVALID_ACTION_PROFILE_NEW_RECORD_TYPE_LINKED_TO_MAPPING_PROFILE =
    "Can not update ActionProfile recordType and linked MappingProfile recordType are different";
  ...
  @Override
  protected Future<Errors> validateProfile(OperationType operationType, ActionProfileUpdateDto profileDto,
                                           String tenantId) {
    return composeFutureErrors(
      ...
      operationType == OperationType.UPDATE ? validateActionProfileChildProfilesFolioRecord(profileDto, tenantId)
  ...
  private Future<Errors> validateActionProfile(OperationType operationType,
                                               ActionProfileUpdateDto actionProfileUpdateDto, String tenantId) {
  ...
  private Future<Errors> validateActionProfileAddedRelationsFolioRecord(ActionProfileUpdateDto actionProfileUpdateDto,
                                                                        String tenantId) {
    ...
    var futures = actionProfileUpdateDto
      ...
      .map(profileAssociation -> profileServiceFactory.getMappingProfileService()
        .getProfileById(profileAssociation.getDetailProfileId(), false, tenantId))
      .map(futureMappingProfile -> futureMappingProfile.onSuccess(optionalMappingProfile ->
        optionalMappingProfile.ifPresent(
          mappingProfile -> validateAssociations(actionProfileUpdateDto.getProfile(), mappingProfile, errors,
  ...
  private Future<Errors> validateActionProfileChildProfilesFolioRecord(ActionProfileUpdateDto actionProfileUpdateDto,
                                                                       String tenantId) {
          ...
          var existMappingProfiles = CollectionUtils.isEmpty(deletedRelations) ? actionProfile.getChildProfiles()
            ...
            existMappingProfiles.forEach(mappingWrapper -> {
              var mappingProfile = DatabindCodec.mapper()
                .convertValue(mappingWrapper.getContent(), MappingProfile.class);
              validateAssociations(actionProfileUpdateDto.getProfile(), mappingProfile, errors,
                INVALID_ACTION_PROFILE_NEW_RECORD_TYPE_LINKED_TO_MAPPING_PROFILE);
            });
            promise.complete(new Errors().withErrors(errors));
          }, () -> promise.fail(
            new NotFoundException(String.format("Action profile with id '%s' was not found", profileId)))
        )
      ).onFailure(promise::fail);
