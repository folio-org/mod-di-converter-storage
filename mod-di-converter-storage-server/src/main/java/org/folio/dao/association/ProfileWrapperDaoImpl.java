...
public class ProfileWrapperDaoImpl implements ProfileWrapperDao {
  ...
  private static final Map<String, String> PROFILE_TYPE_TO_COLUMN;

  private static final String ACTION_PROFILE_ID = "action_profile_id";
  private static final String MAPPING_PROFILE_ID = "mapping_profile_id";
  private static final String MATCH_PROFILE_ID = "match_profile_id";
  private static final String JOB_PROFILE_ID = "job_profile_id";

  static {
    PROFILE_TYPE_TO_COLUMN = new HashMap<>();
    PROFILE_TYPE_TO_COLUMN.put(ProfileType.ACTION_PROFILE.value(), ACTION_PROFILE_ID);
    PROFILE_TYPE_TO_COLUMN.put(ProfileType.MAPPING_PROFILE.value(), MAPPING_PROFILE_ID);
    PROFILE_TYPE_TO_COLUMN.put(ProfileType.MATCH_PROFILE.value(), MATCH_PROFILE_ID);
  ...
  public Future<String> save(ProfileWrapper entity, String tenantId) {
    ...
    String query = format(INSERT_QUERY, convertToPsqlStandard(tenantId), TABLE_NAME,
      PROFILE_TYPE_TO_COLUMN.get(entity.getProfileType().value()));
  ...
  public Future<Boolean> checkIfDataInTableExists(String tenantId) {
    ...
    return pgClientFactory.createInstance(tenantId)
      ...
      .onFailure(
        e -> LOGGER.warn("checkIfDataInTableExists:: Error checking data existence in table {}", TABLE_NAME, e));
  }

  @Override
  public Future<List<ProfileWrapper>> getWrapperByProfileId(String profileId, ProfileType profileType,
                                                            String tenantId) {
    ...
    String query = format(SELECT_QUERY_ON_GETTING_PROFILE_WRAPPER, convertToPsqlStandard(tenantId),
      TABLE_NAME, PROFILE_TYPE_TO_COLUMN.get(profileType.value()));
  ...
  private List<ProfileWrapper> mapResultSetToListProfileWrappers(RowSet<Row> resultSet) {
    List<ProfileWrapper> profileWrappers = new ArrayList<>();
