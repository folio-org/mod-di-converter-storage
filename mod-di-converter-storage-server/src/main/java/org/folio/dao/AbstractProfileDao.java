...
public abstract class AbstractProfileDao<T, S> implements ProfileDao<T, S> {
  ...
  public static final String IS_PROFILE_ASSOCIATED_AS_DETAIL_BY_ID_SQL =
    "SELECT exists (SELECT association_id FROM associations_view WHERE detail_id = '%s')";
  public static final String IS_PROFILE_EXIST_BY_NAME =
    "SELECT jsonb FROM %s WHERE trim(both ' ' from lower(jsonb ->> 'name')) = $1 AND jsonb ->> %s != $2 LIMIT 1;";
  private static final Logger logger = LogManager.getLogger();
  private static final String ID_FIELD = "'id'";

  private final PostgresClientFactory pgClientFactory;

  protected AbstractProfileDao(PostgresClientFactory pgClientFactory) {
  ...
  public Future<S> getProfiles(boolean showHidden, String query, int offset, int limit, String tenantId) {
    ...
    try {
      ...
      pgClientFactory.createInstance(tenantId)
        .get(getTableName(), getProfileType(), fieldList, cql, true, false, result -> {
          if (result.succeeded()) {
  ...
  public Future<Optional<T>> getProfileById(String id, String tenantId) {
    ...
    try {
      pgClientFactory.createInstance(tenantId)
        .get(getTableName(), getProfileType(), new Criterion(idCrit), true, false, result -> {
          if (result.failed()) {
  ...
  public Future<T> updateProfile(T profile, String tenantId) {
    ...
    try {
      ...
      pgClientFactory.createInstance(tenantId)
        .update(getTableName(), profile, new Criterion(idCrit), true, updateResult -> {
  ...
  public Future<Boolean> isProfileExistByName(String profileName, String profileId, String tenantId) {
    ...
    client.selectRead(sql, tuple, reply -> {
      if (reply.succeeded()) {
        ...
        logger.warn("isProfileExistByName:: Error during counting profiles by its name. Profile name {}", profileName,
          reply.cause());
  ...
  public Future<Boolean> isProfileAssociatedAsDetail(String profileId, String tenantId) {
    ...
    pgClientFactory.createInstance(tenantId).select(preparedSql, selectAr -> {
      if (selectAr.succeeded()) {
        ...
        logger.warn("isProfileAssociatedAsDetail:: "
                    + "Error during retrieving associations for particular profile by its id. Profile id {}",
          profileId, selectAr.cause());
  ...
  public Future<Boolean> hardDeleteProfile(String profileId, String tenantId) {
    try {
      ...
      return pgClient.get(getTableName(), getProfileType(), idCrit, false)
        ...
        .onFailure(throwable -> {
          String message =
            format("hardDeleteProfile:: Error during hard delete %s with id: %s", getProfileType().getSimpleName(),
              profileId);
