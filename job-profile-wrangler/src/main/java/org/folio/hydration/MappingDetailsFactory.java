...
public final class MappingDetailsFactory {
  ...
  public static MappingDetail createFoundationHoldingsMappingDetails() {
    return withDiscoverySuppressFalse(
      // Holdings update profiles generated by wrangler match incoming MARC 004
      // against holdings.formerIds[], so seed Holdings must retain that value.
      withFormerIdsSeed(createHoldingsMappingDetails(), "holdings.formerIds[]", "004"),
      "holdings.discoverySuppress");
  }

  public static MappingDetail createFoundationItemMappingDetails() {
    return withDiscoverySuppressFalse(
      // Use a wrangler-owned local field instead of 035$a. FOLIO may add several
      // 035$a values during Instance creation, so reading 035$a would pollute
      // item.formerIds and could make later generated updates match the wrong Item.
      withFormerIdsSeed(createItemMappingDetails(), "item.formerIds[]", "945$f"),
  ...
  private static MappingDetail withFormerIdsSeed(MappingDetail mappingDetail, String formerIdsPath,
                                                 String marcExpression) {
    boolean[] found = {false};
    List<MappingRule> mappingFields = mappingDetail.getMappingFields().stream()
      .map(field -> {
        if (formerIdsPath.equals(field.getPath())) {
          found[0] = true;
          return createFieldWithSubfields(field.getName(), field.getPath(), true,
            createSubfield(0, field.getPath(),
              createSubfieldField("formerId", field.getPath(), true, marcExpression)))
